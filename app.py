...
style_modes = {
    0: "Your style should be casual and laid-back, like a text message to a close friend.",
    1: "Your style should be friendly and approachable, as if you're communicating with a coworker.",
    2: "Your style should be balanced, suitable for general purposes.",
    3: "Your style should be formal and professional, like a business email.",
    4: "Your style should be very formal and sophisticated, akin to an academic paper."
}
...
def ai_assist(text, style):
    style_prompt = style_modes.get(style, "Your style should be balanced, suitable for general purposes.")
...
def gpt_from_preset_and_text_and_style(preset, text, style):
    style_prompt = style_modes.get(style, "Your style should be balanced, suitable for general purposes.")
    prompt = preset + text + "\"\n\n" + style_prompt
    response = prompt_gpt([{"role": "system", "content": prompt}], "gpt-4")
    return response

def formalize_text(text, style):
    return gpt_from_preset_and_text_and_style(formalize_message, text, style)

def niceify_text(text, style):
    return gpt_from_preset_and_text_and_style(niceify_message, text, style)
...
def writing_assistant():
    ...
    continuation = ai_assist(text, style)
...
def formalize():
    ...
    formalized = formalize_text(text, style)
...
def niceify():
    ...
    niceified = niceify_text(text, style)
    
    return jsonify({
        'response': niceified
    }), 200
    
@app.route('/v1/auto', methods=['POST', 'OPTIONS'])
@cross_origin()
def auto():
    data = request.get_json()
    text = data.get('text')
    ...
    text = limit_tokens(text, (8192 - 190))
    style_prompt = "Make sure your response is the following style: " + style_modes.get(style, "Your style should be balanced, suitable for general purposes.")
    prompt = text + "\"\n\n" + style_prompt
    
    gpt_response = prompt_gpt([{"role": "system", "content": text}], "gpt-3.5-turbo")
